...
<h1 class="text-3xl font-bold tracking-tight text-gray-900">Aqui se va a mostrar todas las Categorias</h1>
</div>
</header>
<br>
<div class="container size-11/12 m-auto">
<a href="{{route('cat.crear')}}"><button class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded">Nuevo Registro</button></a>
<br>
<br>
		...
		<tbody class="block md:table-row-group">
			...
			<tr class="bg-gray-300 border border-grey-500 md:border-none block md:table-row">
				...
				<td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
					...
					<button class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 border border-blue-500 rounded"><a href="{{route('cat.mostrar',$produ->id)}}">Ver</a></button>
					<button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 border border-blue-500 rounded"><a href="{{route('cat.editar', $produ)}}">Editar</a></button>

					<form action="{{route('cat.borrar', $produ)}}" method="POST">
						@method('delete')
						@csrf

						@if($produ->deleted_at)

							<button type="button" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 border border-yellow-500 rounded"><a href="{{route('activapro',$produ->id)}}">activar</a></button>
